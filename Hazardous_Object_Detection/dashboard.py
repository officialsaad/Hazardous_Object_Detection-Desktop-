...
class admin:
    ...
    def setcontact(self, c):
        if c == "":
            ...
            if not c.isdigit():
                messagebox.showerror("Invalid Contact", "Contact should contain only digits")
            else:
                self.__contact = c
    ...
    def setquest(self, ques):
        if ques == "":
            ...
            if not all(char.isalpha() or char.isspace() for char in ques):
                messagebox.showerror("Invalid Characters", "Question should contain only alphabets and spaces")
            else:
                self.__ques = ques
    ...
    def setanswer(self, answer):
        if answer == "":
            ...
            messagebox.showerror("Invalid Length", "Answer should be at most 30 characters")
        elif not all(char.isalpha() or char.isspace() for char in answer):
            messagebox.showerror("Invalid Characters", "Answer should contain only alphabets and spaces")
        else:
    ...
    def setpassword(self, password):
        if password == "":
            ...
            messagebox.showerror("Invalid Length", "Password should be at least 8 characters")
        else:
    ...
    def insert(self):
        ...
        param = (self.__name, self.__contact, self.__email, self.__ques, self.__answer, self.__password)
    ...
    def update(self):
        query = 'UPDATE registration SET name = ?, contact_no = ?, email = ?, question = ?, answer = ?, password = ? WHERE id = ?'
        params = (self.__name, self.__contact, self.__email, self.__ques, self.__answer, self.__password, self.__id)
...
class dashboard:
    ...
    def create_log_frame(self):
        ...
        self.left_frame = ctk.CTkFrame(self.root, fg_color="#D4CDCE", corner_radius=10)
        self.left_frame.pack(side="left", fill="y", padx=(20, 10), pady=(15, 15))
        ...
        sframe = ctk.CTkScrollableFrame(self.left_frame, width=200, height=600, fg_color="white")
    ...
    def verification_password(self):
        ...
        if ques == "select":
            ...
            if email == "":
                ...
                db = DatabaseConnection()
                db.connect()
                result = db.execute_and_fetch_query(query, True, (email, ques, answer))
                if result:
    ...
    def create_right_frame(self):
        self.frame2 = ctk.CTkFrame(self.root, fg_color="#B3B6B7", width=200, height=400, corner_radius=10)
        self.frame2.pack(side="left", expand="true", fill="both", padx=(0, 20), pady=(15, 15))

    def create_left_frame(self):
        self.left_frame = ctk.CTkFrame(self.root, fg_color="#B3B6B7", corner_radius=10)
        self.left_frame.pack(side="left", fill="y", padx=(20, 10), pady=(15, 15))
    ...
    def show_images_on_dash(self):
        ...
        img_frame = ctk.CTkFrame(self.frame2, corner_radius=30, fg_color="white")
        img_frame.pack(anchor="center", expand="true", padx=(20, 20), pady=(20, 20), fill="both")

        frame2 = ctk.CTkFrame(img_frame, corner_radius=30, height=60, fg_color="white")
        ...
        delete_btn.pack(side="left", padx=(10, 10), pady=(10, 10), expand="true")
        ...
        delete_all.pack(side="left", padx=(10, 10), pady=(10, 10), expand="true")
    ...
    def prev_rotate_img(self):
        if self.counter == 0:
            ...
            self.counter -= 1
            self.img_label.configure(image=self.img_array[self.counter % len(self.img_array)])
    ...
    def show_imag(self, frame2):
        ...
        self.img_array = []
        for file in files:
            img = PIL.Image.open(os.path.join('images', file))
            img_resize = img.resize((850, 450))
            self.img_array.append(ImageTk.PhotoImage(img_resize))

        print(self.img_array)

        self.img_label = ctk.CTkLabel(frame2, text="", image=self.img_array[0], corner_radius=30, )
    ...
    def create_theme_frame(self, frames):
        ...
        theme_frame = ctk.CTkFrame(frames, border_width=1, corner_radius=0, width=350, height=400)
        ...
        theme_color_label = CTkLabel(theme_frame, corner_radius=0, text="Themes Colors")
        theme_color_label.place(relheight=0.2, relx=0.5, anchor="center", rely=0.48)
    ...
    def changetheme(self):
        if self.theme_color_default == "":
            ...
            self.root.destroy()
            roots = ctk.CTk()
            ctk.set_default_color_theme(self.theme_color_default)
            root(roots)
            roots.mainloop()
    ...
    def registration_data(self):
        user = admin()
        user.setname(self.txt_fname.get().strip().lower() + " " + self.txt_lname.get().strip().lower())
        user.setemail(self.txt_email.get().strip().lower())
        if self.txt_pass.get() == self.txt_cpass.get() and len(self.txt_pass.get()) == len(self.txt_cpass.get()):
            ...
            messagebox.showerror("Password Matching Error", "Password and Confirm Password did not Match")
    ...
    def update_data(self):
        user = admin()
        user.setname(f"{self.txt_fname.get().strip().lower()} {self.txt_lname.get().strip().lower()}")
        user.setemail(self.txt_email.get().strip().lower())
        if self.txt_pass.get() == self.txt_cpass.get() and len(self.txt_pass.get()) == len(self.txt_cpass.get()):
            user.setpassword(self.txt_pass.get().strip())
            user.setquest(self.cmb_ques.get().strip().lower())
            ...
            result = user.update()
            if result:
                messagebox.showinfo("Message", "Sucessfully Update user details")
            else:
    ...
    def user_data(self):
        ...
        self.db.connect()
        result = self.db.execute_and_fetch_query(query=query, fetch=True, params=param)

        for index, row in enumerate(result):
            if index == 0:  # Check if it's the first tuple in the list
                name_parts = row[1].split()  # Split the name at the second index
